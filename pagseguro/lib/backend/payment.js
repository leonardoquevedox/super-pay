    ...
    create: (transaction) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let credentials = { email: config.api_email, token: config.api_token };
                let url = config.checkout_url + "/transactions?" + querystring.stringify(credentials);
                let content = new Js2Xml("payment", payment);
                let response = (await axios.post(url, content.toString(), { headers: { "Content-Type": "application/xml; charset=ISO-8859-1" } })).data;
                resolve(response);
            } catch (e) {
                if (e.response && e.response) {
                    let response = e.response.data;
                    let xml = xmlJS.xml2json(response, { compact: true, spaces: 4 });
                    reject(xml);
                } else {
                    reject(e.message);
                }
